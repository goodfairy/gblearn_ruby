...
puts 'press enter'
gets
puts '4. *Дан массив %i(first second third). Превратите его в хэш {first: 1, second: 2, third: 3}.'
...
class Object
  ...
  def my_map
    outarr = []
    if block_given? then each { |element| outarr.push(yield(element)) } # then self.each
    else
      outarr = to_enum :my_map
    end
    outarr
  end
end
...
arraytodo = [1, 2, 3]
double = proc { |num| num * 2 } # double = Proc.new {|num| num * 2}
